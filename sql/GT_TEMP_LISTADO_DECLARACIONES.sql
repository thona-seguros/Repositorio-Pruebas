      ...
      IF OC_ASEGURADO.EDAD_ASEGURADO(nCodCia, nCodEmpresa, nCodAsegurado, dFecIniVig) < NVL(W.Edad_Exclusion,0) THEN
         ...
         BEGIN
            ...
            ELSIF NVL(W.IndCuotaPromedio,'N') = 'S' AND NVL(W.CuotaPromedio,0) != 0 THEN
               ...
               nCuotaPromedio := (GT_COTIZACIONES_COBERT_MASTER.PRIMA_COBERTURA(nCodCia, nCodEmpresa, nIdCotizacion, nIDetCotizacion, W.CodCobert)) / nValorTipoTasa;
...
cCodCobrador      FACTURAS.CodCobrador%TYPE;
CURSOR FACT_Q IS
   SELECT IdFactura
     FROM FACTURAS
    WHERE IdPoliza  = nIdPoliza
...
BEGIN
   ...
   FOR W IN CANCFACT_Q LOOP
      ...
      SELECT NVL(nTotPrimaCanc,0) + NVL(SUM(Saldo_Det_Moneda),0), 
              F.NumCuota, F.CodCobrador
        INTO nTotPrimaCanc,
             nNumCuota, cCodCobrador
       ...
       GROUP BY F.NumCuota, F.CodCobrador;
      
      nNumCuotaIni := nNumCuota;

      OC_FACTURAS.ANULAR(nCodCia, W.IdFactura, TRUNC(SYSDATE),'PAD', cCodCobrador, nIdTransaccion);
...
-- Grants for Package
GRANT EXECUTE ON GT_TEMP_LISTADO_DECLARACIONES TO PUBLIC
/
